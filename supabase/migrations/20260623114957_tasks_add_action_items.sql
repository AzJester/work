-- Per-task checkable action items, stored inline as JSON so they survive the
-- delete-and-reinsert save strategy (each element: { "text": "...", "done": true|false }).
alter table public.tasks add column if not exists items jsonb not null default '[]'::jsonb;
        ...
        select rep.week_ending, rep.prepared_by,
          coalesce((
            select json_agg(t) from (
              select task, status, priority, progress, note, position, project, due_date, items
              from public.tasks
