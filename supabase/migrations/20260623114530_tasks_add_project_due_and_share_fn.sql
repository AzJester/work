-- New optional per-task fields for the daily/weekly workflow.
alter table public.tasks add column if not exists project  text not null default '';
alter table public.tasks add column if not exists due_date date;

-- Recreate the read-only shared-dashboard payload to include project + due_date.
        ...
        select rep.week_ending, rep.prepared_by,
          coalesce((
            select json_agg(t) from (
              select task, status, priority, progress, note, position, project, due_date
              from public.tasks
